-- ============================================================
-- Lower the minimum-sample thresholds on analytics_insights so
-- early-stage data (n=1 or n=2) is visible during development.
--
-- IMPORTANT: this is a dev-friendly setting. Once real users are
-- in the system, raise thresholds back to >= 10 (or filter
-- queries with `WHERE sample_size >= 10`) before publishing
-- anything in a buyer-facing report. n=1 insights describe one
-- user's experience and should never be presented as a trend.
--
-- Re-creates the view from migration 002 with all HAVING clauses
-- relaxed to >= 1. Also wraps every `payload->>'key'` in
-- parentheses to defend against an operator-precedence quirk
-- where `||` binds before `->>` and tries to coerce literal
-- strings into JSONB (fails with "invalid input syntax for type
-- json"). Migration 002 happened to work without parens on the
-- old Postgres version; current version requires them.
-- ============================================================
