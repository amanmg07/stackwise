-- ============================================================
-- Fix the `stacks` insight in analytics_insights.
--
-- Bug (reported from real TestFlight data, cycle f240cdbe):
-- the stacks branch read ONLY `cycle_created` events and grouped
-- by the creation-time peptide_ids snapshot, so a stack edited
-- after creation (e.g. user created creatine+tesamorelin, then
-- removed tesamorelin) kept reporting the original composition.
--
-- This rewrite fixes three things in that branch:
--   1. Effective composition: take the LATEST cycle_created OR
--      cycle_updated event per cycle_id, not the creation snapshot.
--      A cycle edited down to a single compound correctly drops
--      out of "stacks" entirely.
--   2. Order-independent: normalize the peptide set (sorted +
--      de-duplicated) so [A,B] and [B,A] are the same stack.
--   3. "used by N users" now counts DISTINCT users, not raw
--      event count.
--
-- Non-destructive: CREATE OR REPLACE VIEW preserves grants and
-- changes no data — only the stacks branch logic. All 13 other
-- branches are reproduced verbatim from the live definition.
-- The long-term answer remains a materialized stack_combinations
-- table with a combination_hash (architecture-spec gap #4); this
-- keeps the interim view honest until then.
...
UNION ALL
 SELECT 'stacks'::text AS category,
    ((('Stack '::text || (norm.peptide_set)::text) || ' used by '::text) || count(DISTINCT norm.anon_id)) || ' users'::text AS insight,
