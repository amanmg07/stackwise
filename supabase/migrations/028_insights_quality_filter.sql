-- ============================================================
-- analytics_insights: apply the migration-006 "abandoned cycle"
-- quality filter to the cycle-derived branches.
--
-- Background: migration 006 established that a cycle the user
-- created and never logged a single dose for is noise, and filtered
-- it out of the buyer-facing dataset — but the filter was only added
-- to research_cycles_started / research_cycles_ended, NOT to
-- analytics_insights. As a result a solo/never-tracked cycle still
-- produced buyer-facing insights ("X run solo by 1 user", demographic
-- and retention rows) until it was deleted, contradicting 006's
-- stated intent of keeping mistakes/noise out of the buyer dataset.
--
-- This migration brings analytics_insights in line with 006: every
-- branch derived from cycle_created / cycle_updated / cycle_ended now
-- requires that the same user logged >=1 dose_logged for that
-- cycle_id. dose_logged / journal_entry / scan_compared /
-- user_profiles branches are unchanged (they are real tracked
-- signal, not abandoned-cycle noise). This also makes the in-app
-- cascade-delete a backstop rather than the only line of defense.
--
-- Verified: cycle_created, cycle_updated, cycle_ended and dose_logged
-- payloads all carry 'cycle_id'; dose_logged carries anon_id — the
-- EXISTS join keys line up exactly with 006's research_cycles_* form,
-- so there is no risk of zeroing out a branch.
--
-- Builds on migration 026 (solo-inclusive stacks, effective
-- composition). Non-destructive CREATE OR REPLACE VIEW; the 9
-- non-cycle branches are reproduced verbatim from 026.
--
-- Post-deploy sanity check (should drop noise rows, keep tracked):
--   SELECT category, insight FROM analytics_insights
--   WHERE category IN ('stacks','retention','demographics')
--   ORDER BY category;
-- ============================================================
 ...
 SELECT 'retention'::text AS category,
  ...
  WHERE e.event_type = 'cycle_ended'::text
    AND (EXISTS ( SELECT 1
          ...
          WHERE d.event_type = 'dose_logged'::text
            AND (d.payload ->> 'cycle_id'::text) = (e.payload ->> 'cycle_id'::text)
